...
#             Pre_existing_blood_pressure,
             pre_existing_BP = ifelse(Pre_existing_blood_pressure == 'Low', 1, 
                                      ifelse(Pre_existing_blood_pressure == 'Avg', 1, 2)),
             B4_Med_BP_Sys,
             After_Med_BP_Sys,
             B4_color_BP_Sys,
             After_color_BP_Sys) ]
...
legend(x=98,y=147, legend=c("Group 1: meditation", "Group 0: coloring"), col=cols_t1, pch=16, xpd=TRUE)
...
```{r}
# On meditation variables & group interaction
m1 <- lm(After_Med_BP_Sys ~ pre_existing_BP + Years_practice + hours_since_last_caffeinated_drink + B4_Med_BP_Sys + Group + B4_Med_BP_Sys * Group, data=d2)
coeftest(m1, vcovHC(m1))
# On coloring variables & group interaction
m2 <- lm(After_color_BP_Sys ~ pre_existing_BP + Years_practice + hours_since_last_caffeinated_drink + B4_color_BP_Sys + Group + B4_color_BP_Sys * Group, data=d2)
coeftest(m2, vcovHC(m2))
```

##### Can't generalize from this finding because:
##### Subjects are our friends and relatives. This sample is not randomized from the general population. 